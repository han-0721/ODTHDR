...
for epoch in range(start_epoch, opt.epochs + 1):
    ...
    for batch, data in enumerate(tqdm(data_loader, desc="Batch %")):
        ...
        # hdr_drago_tonemap = F.normalize(hdr_drago_tonemap, mean=[0.5], std=[0.5])
        ...
        # mse_sdr_loss = Fct.mse_loss(sdr_out, drago_tonemap_gt, reduction="mean")
        # mse_hdr_loss = Fct.mse_loss(hdr_drago_tonemap, drago_tonemap_gt, reduction="mean")
        ...
        # loss_sdr = l1_sdr_loss + 0.5 * ssim_sdr_loss + 0.1 * mse_sdr_loss
        loss_hdr = l1_hdr_loss + 0.5 * l1_hdr_drago_tonemap_loss + 0.4 * ssim_hdr_loss + 0.2 * vgg_loss
        loss = loss_hdr
        # loss = loss_hdr + 0.4 * loss_sdr
    ...
    # writer.add_scalar('mse_sdr_loss', mse_sdr_loss_sum/idx, epoch)
    # writer.add_scalar('mse_hdr_loss', mse_hdr_loss_sum/idx, epoch)
    ...
    if epoch % opt.save_ckpt_after == 0:
        # save_checkpoint(epoch, diffmodel, "../autodl-tmp/diffhdr/")
        save_checkpoint(epoch, mainmodel, "../autodl-tmp/mainhdr/")
